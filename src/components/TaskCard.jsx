...
function TaskCard({ task, setTasks }) {
  ...
  return (
    <div 
      ...
      className={`bg-slate-800/60 group rounded-2xl p-4 border border-white/5 hover:border-indigo-500/50 cursor-grab active:cursor-grabbing touch-none ${
        isDragging ? 'opacity-20 scale-95 ring-2 ring-indigo-500/50' : 'opacity-100'
      }`}
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex justify-end items-center md:opacity-30 opacity-100">
           <GripVertical size={14} className="text-slate-500" />
        </div>
      </div>
    </div>
